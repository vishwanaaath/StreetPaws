...
const EditDog = () => {
  ...
  const [gender, setGender] = useState(dogData.gender);
  const [isGenderDropdownOpen, setIsGenderDropdownOpen] = useState(false);
  const [isLoaded, setIsLoaded] = useState(false);
  const [age, setAge] = useState(dogData.age); // Update state initialization
  const [email, setEmail] = useState(dogData.email);
  const [phone, setPhone] = useState(dogData.phone);
  ...
  // Update validateContactInfo function
  const validateContactInfo = () => {
    const newErrors = {};

    // Use state values directly since they already contain user data
...
const handleSubmit = async (e) => {
  ...
  try { 
    ...
    // Create updated dog data object
    const updatedDogData = {
      ...
      location: {
        type: "Point",
        coordinates: [
          geoLocation.lng, // Longitude first
          geoLocation.lat, // Latitude second
        ],
      },
      age,
      gender,
    ...
    const response = await axios.patch(
      `${import.meta.env.VITE_API_URL}/api/dogs/${dogData._id}`,
      updatedDogData,
      {
        headers: {
    ...
    // Handle success
    setNotificationImage(`${prefix}${imageUrl}`);
    setNotificationMessage("Dog updated successfully!");
            ...
            <div className="mb-6">
              ...
              {geoLocation && (
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  <p className="text-sm">
                    previously captured: {geoLocation[0]}, {geoLocation[1]}
                  </p>
                </div>
              )}
            </div>
